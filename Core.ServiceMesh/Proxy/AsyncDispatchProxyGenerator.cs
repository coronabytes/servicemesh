...
internal static class AsyncDispatchProxyGenerator
{
    ...
    private static Type GetProxyType(Type baseType, Type interfaceType)
    {
        ...
        {
            Dictionary<Type, Type>? interfaceToProxy;
    ...
    private static Type GenerateProxyType(Type baseType, Type interfaceType)
    {
        ...
        foreach (var t in interfaceType.GetTypeInfo().ImplementedInterfaces) pb.AddInterfaceImpl(t);
    ...
    private static ProxyMethodResolverContext Resolve(object[] args)
    {
        ...
        if (method.IsGenericMethodDefinition) method = ((MethodInfo)method).MakeGenericMethod(packed.GenericTypes);
    ...
    private class ProxyBuilder
    {
        ...
        internal void AddInterfaceImpl(Type iface)
        {
            ...
            foreach (var pi in iface.GetRuntimeProperties())
            {
                ...
                if (pi.GetMethod != null) propertyMap[pi.GetMethod] = ai;

                if (pi.SetMethod != null) propertyMap[pi.SetMethod] = ai;
            }
            ...
            foreach (var ei in iface.GetRuntimeEvents())
            {
                ...
                if (ei.AddMethod != null) eventMap[ei.AddMethod] = ai;

                if (ei.RemoveMethod != null) eventMap[ei.RemoveMethod] = ai;

                if (ei.RaiseMethod != null) eventMap[ei.RaiseMethod] = ai;
            }
            ...
            foreach (var pi in iface.GetRuntimeProperties())
            {
                ...
                if (ai.GetMethodBuilder != null) pb.SetGetMethod(ai.GetMethodBuilder);

                if (ai.SetMethodBuilder != null) pb.SetSetMethod(ai.SetMethodBuilder);
            }
            ...
            foreach (var ei in iface.GetRuntimeEvents())
            {
                ...
                if (ai.AddMethodBuilder != null) eb.SetAddOnMethod(ai.AddMethodBuilder);

                if (ai.RemoveMethodBuilder != null) eb.SetRemoveOnMethod(ai.RemoveMethodBuilder);

                if (ai.RaiseMethodBuilder != null) eb.SetRaiseMethod(ai.RaiseMethodBuilder);
            }
        }
        ...
        private static Type[] ParamTypes(ParameterInfo[] parms, bool noByRef)
        {
            ...
            for (var i = 0; i < parms.Length; i++)
            {
                ...
                if (noByRef && types[i].IsByRef) types[i] = types[i].GetElementType()!;
            }
        ...
        {
            if (type == null) return 0; // TypeCode.Empty;

            if (type == typeof(bool)) return 3; // TypeCode.Boolean;

            if (type == typeof(char)) return 4; // TypeCode.Char;

            if (type == typeof(sbyte)) return 5; // TypeCode.SByte;

            if (type == typeof(byte)) return 6; // TypeCode.Byte;

            if (type == typeof(short)) return 7; // TypeCode.Int16;

            if (type == typeof(ushort)) return 8; // TypeCode.UInt16;

            if (type == typeof(int)) return 9; // TypeCode.Int32;

            if (type == typeof(uint)) return 10; // TypeCode.UInt32;

            if (type == typeof(long)) return 11; // TypeCode.Int64;

            if (type == typeof(ulong)) return 12; // TypeCode.UInt64;

            if (type == typeof(float)) return 13; // TypeCode.Single;

            if (type == typeof(double)) return 14; // TypeCode.Double;

            if (type == typeof(decimal)) return 15; // TypeCode.Decimal;

            if (type == typeof(DateTime)) return 16; // TypeCode.DateTime;

            if (type == typeof(string)) return 18; // TypeCode.String;

            if (type.GetTypeInfo().IsEnum) return GetTypeCode(Enum.GetUnderlyingType(type));
        ...
        private sealed class MethodInfoEqualityComparer : EqualityComparer<MethodInfo>
        {
            ...
            public override bool Equals(MethodInfo? left, MethodInfo? right)
            {
                ...
                if (!Equals(left.DeclaringType, right.DeclaringType)) return false;

                if (!Equals(left.ReturnType, right.ReturnType)) return false;

                if (left.CallingConvention != right.CallingConvention) return false;

                if (left.IsStatic != right.IsStatic) return false;

                if (left.Name != right.Name) return false;

                var leftGenericParameters = left.GetGenericArguments();
                var rightGenericParameters = right.GetGenericArguments();
                if (leftGenericParameters.Length != rightGenericParameters.Length) return false;
